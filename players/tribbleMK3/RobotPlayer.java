...
public class RobotPlayer {
	...
	public static void run(RobotController tribbles) {
		rc = tribbles;
		rand = new Random();
		...
		while(true) {
			...
			if (rc.getType() == RobotType.HQ) {
				try {
					...
					rc.broadcast(50, numMinerFactory);
					rc.broadcast(60, numMiner);
					rc.broadcast(100, numBarracks);
					rc.broadcast(150, numSupplyDepot);
			...
			if (rc.getType() == RobotType.BEAVER) {
				try {
					if (rc.isWeaponReady()) {
						...
						int fate = rand.nextInt(1000);
						if (fate < 20 && rc.getTeamOre() >= 300) {
							tryBuild(directions[rand.nextInt(8)],RobotType.BARRACKS);
						} else if (fate < 950 && rc.getTeamOre() >= 300) {
			...
			if (rc.getType() == RobotType.MINER) {
					...
					if (rc.isWeaponReady()) {
						...
						if (rc.isCoreReady()) {
							...
							else tryMove(rc.senseHQLocation().directionTo(rc.getLocation()));
						}
				} catch (Exception e) {
